...
import { getMisCitasTerapeuta, getMisPacientes } from '../services';

export default function TerapeutaHomePage() {
  ...
  const citasHoy = misCitas.filter(cita => {
  ...
  return (
    <div className="p-6">
      <div className="mb-8">
        ...
        <p className="text-gray-600">Gestiona tus pacientes y citas</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-600 text-sm font-medium">Mis Pacientes</p>
              <p className="text-3xl font-bold text-gray-900">{misPacientes.length}</p>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900">{misCitas.length}</p>
            </div>
      ...
      {/* Citas de hoy y Próximos pacientes */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              citasHoy.map(cita => (
                <div key={cita.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">{cita.motivo || 'Consulta general'}</p>
                  </div>
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6">
          ...
          <div className="space-y-3">
            {misPacientes.slice(0, 5).map(paciente => (
            ...
            {misPacientes.length === 0 && (
              <p className="text-gray-500 text-center py-4">No tienes pacientes asignados</p>
            )}
          </div>
        </div>
      </div>
